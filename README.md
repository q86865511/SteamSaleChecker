...
| 設定 | `/settings` | 主題(跟隨系統/深/淺)、語言(繁中/英)、預設檢視;登入後的通知偏好(降價/免費/摘要/類型/頻道·DM) |
...
5. **通知(選配)** —— 對「收藏且跌破先前最低(或設定的目標價)」的遊戲、新出現的免費遊戲、與每日/每週摘要,由 Discord Bot 依 per-user 偏好送到頻道 @你 或私訊 DM。
...
- **個人化通知偏好**:降價 / 免費 / 每日·每週摘要訂閱、**只接收特定類型**、通知**送共用頻道 @你 或私訊 DM**;摘要頻率 per-user,不必改 env。
...
3. **摘要**:`SSC_DIGEST_HOURS`(0=停用、24=每日、168=每週);需 bot token + notify channel。使用者也可在 `/settings` 自選摘要頻率(per-user,不必改 env)。
...
| `DISCORD_REDIRECT_URI` | `http://localhost:8787/auth/callback` | OAuth callback;上線換正式網域 |
...
- CI/CD:[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml)(`appleboy/ssh-action`)—— push `main` → SSH 到 VM → `git reset --hard` → `npm ci` → build web → `rsync` 靜態檔到 `/srv/steam`(`--exclude data`)→ `docker compose up -d --build` → `curl /health` 健康檢查(最多 15 次)。
...
- **CI 只負責部署,無自動化前端測試**:GitHub Actions 只有 `deploy.yml`;單元測試(164)需本機 `npm test`,前端以瀏覽器實測。README 徽章中的 tests 數為**靜態**標示。
